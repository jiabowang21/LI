...
writeClauses(MaxConsecutiveHours):-
    ...
    noGangsterCanDoTwoDifferentTaskDuringSameHour,
    
    % No gangster can do two different tasks during two consecutive hours
    noGangsterCanDoTwoDifferentTaskDuringTwoConsecutiveHours,
    
    % All tasks need to be planned
    allTasksPlanned,
    
    % no gangster works more than K consecutive hours
    noGangsterWorksMoreThanKConsecutiveHours(MaxConsecutiveHours),
...
noGangsterCanDoTwoDifferentTaskDuringTwoConsecutiveHours:-
    ...
    available(G, H2),
    task(T), task(T2), T \= T2,
    writeClause( [-does(G,T,H), -does(G,T2,H2)] ),
    fail.
...
noGangsterWorksMoreThanKConsecutiveHours(MaxConsecutiveHours):-
    gangster(G),
    AuxH is 72 - MaxConsecutiveHours,
    between(1,AuxH, H1),
    \+blocked(G,H1),
    H2 is H1 + MaxConsecutiveHours, hour(H2),
    findall(-busy(G,H), between(H1, H2, H), Lits),
    writeClause(Lits),
    fail.
    
noGangsterWorksMoreThanKConsecutiveHours(_).

% busy -> does V does V does
busyImpliesDoes:-
    available(G,H),
    findall(does(G,T,H), task(T), Lits),
