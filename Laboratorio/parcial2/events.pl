...
writeClauses:-  
    %.... %% Complete this!  
    % Each event should have a moderator assigned to it
    %eachEventOneDay,
    ...
    % There is a limit on the number of events per day 
    limitEventsPerDay,
    
    % There is a limit on the amount of days where events with the same moderator can take place.
...
eachEventOneModerator:- eventModerators(E, Mo), findall( em(E, M), member(M, Mo), Lits ), exactly(1, Lits), fail. 
eachEventOneModerator.

eachEventOneDay:- eventDays(E, Do), findall( ed(E, D), member(D, Do), Lits ), exactly(1, Lits), fail.
eachEventOneDay.

limitEventsPerDay:- day(D), maxEventsPerDay(Max), findall( ed(E, D), event(E), Lits ), atMost(Max, Lits), fail. 
limitEventsPerDay.

limitDaysPerModerator:- moderator(M), maxDaysPerModerator(Max), findall( md(M,D), day(D), Lits ), atMost(Max, Lits), fail.
limitDaysPerModerator.

relation:- event(E), day(D), moderator(M), writeClause([ -ed(E, D), -em(E, M), md(M, D) ]), fail.
relation.
