...
writeClauses:-  
    ...
    %departs from there for the next traject the next day
    departsFromDestination,
    
    %Each traject takes place at most once per day
    trajectAtMostOncePerDay,
    
    %Each traject takes place at least a given minimal amount of days per week
    trajectAtLeastNPerWeek,
    
    %No bus can travel more than a total of a given amount of kms on two consecutive days
    noMorethanLimitConsecutiveDays,
...
departsFromDestination:- bus(B), consecutiveDays(D1, D2), trip(C1-C2), trip(C3-C4), C2 \= C3, writeClause([ -bdcc(B, D1, C1, C2), -bdcc(B, D2, C3, C4) ]), fail.
departsFromDestination.

trajectAtMostOncePerDay:- day(D), trip(C1-C2), findall( bdcc(B, D, C1, C2), bus(B), Lits ), atMost(1, Lits), fail.
trajectAtMostOncePerDay.

trajectAtLeastNPerWeek:- frequency(C1-C2, F), findall( bdcc(B, D, C1, C2), (bus(B), day(D)), Lits ), atLeast(F, Lits), fail.
trajectAtLeastNPerWeek.

noMorethanLimitConsecutiveDays:- tooLongDist(C1-C2-C3), bus(B), consecutiveDays(D1, D2), writeClause([ -bdcc(B, D1, C1, C2), -bdcc(B, D2, C2, C3) ]), fail.
noMorethanLimitConsecutiveDays.
