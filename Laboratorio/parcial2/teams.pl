...
incompatibleWorkers(W1,W2):- workerScore(W1, S1), workerScore(W2, S2), maxScore(Max), SS is S1 + S2, SS > Max. 
...
writeClauses:-  
    %.... %% Complete this! 
    % working teams of size between minSize and maxSize
    eachTeamSizeBetweenMinAndMax,
    
    % each worker is at most in one team
    eachWorkerAtMostInOneTeam,
    
    %It has been decided that two workers whose scores sum more than maxScore cannot be in the same team.
    incompatibleWorkers,
    
    true,!.
writeClauses:- told, nl, write('writeClauses failed!'), nl,nl, halt.

eachTeamSizeBetweenMinAndMax:- team(T), minSize(Min), maxSize(Max), findall( wt(W, T), worker(W), Lits ), atMost(Max, Lits), atLeast(Min, Lits), fail. 
eachTeamSizeBetweenMinAndMax.

eachWorkerAtMostInOneTeam:- worker(W), findall( wt(W, T), team(T), Lits), exactly(1, Lits), fail.
eachWorkerAtMostInOneTeam.

incompatibleWorkers:- incompatibleWorkers(W1, W2), team(T), W1 \= W2, writeClause([ -wt(W1, T), -wt(W2, T)]), fail.
incompatibleWorkers.
