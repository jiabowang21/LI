...
noDoubles:- 
    noDoubles(ND), round(R), member(R, ND), findall( double(S, R), team(S), Lits ), exactly(0, Lits),
    fail.
noDoubles.

%Movistar has bought the tv rights for Saturday Night 8pm for all matches among a group of teams (the so-called tvTeams) and wants all matches among these teams on different rounds (i.e., no two of them on the same round).
atmostOneTVMatchPerRound:- 
    round(R), findall( match(S,T,R), tvMatch(S-T), Lits ), atMost(1, Lits),
    fail.
atmostOneTVMatchPerRound.

%Each team gets either 6 or 7 home matches.
sixOrSevenHomes:-  
    team(S), findall( home(S, R), round(R), Lits ), atMost(7, Lits), atLeast(6, Lits),
    fail.
sixOrSevenHomes.

%No team gets more than one double in the whole league
atMostOneDouble:-  
    team(S), findall( double(S, R), round(R), Lits ), atMost(1, Lits), 
    fail.
atMostOneDouble.

writeDouble:- round(R), R > 1, R1 is R - 1, team(T), writeClause([ home(T, R), home(T, R1), double(T, R) ]), writeClause([ -home(T, R), -home(T, R1), double(T, R) ]), fail.
writeDouble.
