...
p:-	ejemplo1(RowSums,ColSums),
	...
	length(RowSums,NumRows),   %NumRows = numero de filas
	
	length(ColSums,NumCols),   %NumCols = numero de columnas
	
	%1: Dominio:
	NVars is NumRows*NumCols,  %Numero de variables = numero de filas * numero de columnas
	
	%2: Constraints:
	listVars(NVars, L),                                % generate a list of Prolog vars (their names do not matter)
	matrixByRows(L, NumCols, MatrixByRows),            % genera una matriz por filas a partir de L
    transpose(MatrixByRows, TransposedMatrix),         % transpose the matrix 
    declareConstraints(MatrixByRows, TransposedMatrix, RowSums, ColSums, L),
    
    %Escribimos el resultado:
    pretty_print(RowSums, ColSums, MatrixByRows).

listVars(NVars, L):- length(L, NVars).      %genera una lista L con NVars variables 
    
% Create a Matrix of NumCols columns from all variables in the list.
matrixByRows(L, NumCols, MatrixByRows):-
            length(L, Lsize),               %mida de L
            NumRows is Lsize div NumCols,   %NumRows = Lsize / NumCols
            length(MatrixByRows, NumRows),  %genera una lista MatrixByRows con NumRows variables 
