...
% Every day a bus does exactly one traject
...
writeClauses:-
    %Every day a bus does exactly one traject (according to the trajects given in the input) from one city to another one, and departs from there for the next traject the next day.
    exactlyOneTripPerBusAndDay,
    
    %Each traject takes place at most once per day, and at least a given minimal amount of days
    atMostOncePerDay,
    %atMostOneTrajectPerDay,
    
    %Each traject takes place at least a given minimal amount of days
    atLeastNPerDay,
    %eachTrajectAtLeastMinimum,
    
    %From one city to another one, and departs from there for the next traject the next day
    nextDayDepartsFromDestination,
    
    %No bus can travel more than a total of a given amount of kms on two consecutive days.
    maxKmConsecutiveDays,
    %% ...
    true,!.
...
atMostOncePerDay:- trip(C1-C2), day(D), findall( bdcc(B, D, C1, C2), bus(B), Lits ), atMost(1, Lits), fail.
atMostOncePerDay. 

atLeastNPerDay:- frequency(C1-C2, F), findall( bdcc(B, D, C1, C2), (bus(B), day(D)), Lits ), atLeast(F, Lits), fail. 
atLeastNPerDay.

nextDayDepartsFromDestination:- bus(B), day(D1), day(D2), consecutiveDays(D1, D2), trip(C1-C2), trip(C3-C4), C2 \= C3, writeClause([-bdcc(B, D1, C1, C2), -bdcc(B, D2, C3, C4)]), fail.
nextDayDepartsFromDestination.

maxKmConsecutiveDays:- bus(B), consecutiveDays(D1, D2), tooLongDist(C1-C2-C3), writeClause([-bdcc(B, D1, C1, C2), -bdcc(B, D2, C2, C3)]), fail. 
maxKmConsecutiveDays.
