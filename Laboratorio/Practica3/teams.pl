...
incompatibleWorkers(W1,W2):- worker(W1), worker(W2), maxScore(M), score(W1, S1), score(W2, S2), SS is S1+S2, SS > M.  %% Complete this!
...
eachTeamAtLeast:- team(T), minSize(Min), findall( wt(W,T), worker(W), Lits), atLeast(Min, Lits), fail.
eachTeamAtLeast.

eachTeamAtMost:- team(T), maxSize(Max), findall( wt(W,T), worker(W), Lits), atMost(Max, Lits), fail.
eachTeamAtMost.

workersCompatible:- worker(W1), worker(W2), team(T), incompatibleWorkers(W1, W2), W1 \= W2, writeClause([-wt(W1,T), -wt(W2,T)]), fail.
workersCompatible.
