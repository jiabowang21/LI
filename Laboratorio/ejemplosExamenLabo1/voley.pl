...
noDoubles:-  
    noDoubles(Nd),
    findall( double(S, R), (member(R, Nd), team(S)) ,Lits ),
    exactly(0, Lits),
    fail.
noDoubles.
...
relation:-
    team(S), round(R), R > 1, R2 is R - 1, writeClause([-home(S, R), -home(S, R2), double(S, R)]), writeClause([home(S, R), home(S, R2), double(S, R)]),
    fail.
relation.
