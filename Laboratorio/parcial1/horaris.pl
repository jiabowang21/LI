...
% each course can have at most one-hour lecture per day
...
%% Consider that each course can have at most one-hour lecture per day
eachCourseRightNumberOfDays:- courseHours(C,Hs), findall( cd(C,D), day(D), Lits ), exactly(Hs,Lits), fail.
eachCourseRightNumberOfDays.

% two courses from the same year cannot overlap in time
noOverlapCoursesSameYear:- courseYear(C,Y), courseYear(C2,Y), C < C2, day(D), hour(H), 
                            writeClause( [ -cdh(C,D,H), -cdh(C2,D,H) ] ), fail.
noOverlapCoursesSameYear.

%no professor can simultaneously teach two courses
...
% two courses cannot be simultaneously held at the same room
noOverlapRooms:- courseRooms(C1, R1), courseRooms(C2, R2), member(R,R1), member(R,R2), C1 < C2, day(D), hour(H),
                            writeClause( [-cdh(C1,D,H), -cdh(C2,D,H), -cr(C1,R), -cr(C2,R) ] ), fail.
noOverlapRooms.

relateVarsCPwithUsedProf:- courseProfessors(C,LP), member(P, LP), 
                            writeClause( [-cp(C,P), usedProf(P) ] ), fail.
relateVarsCPwithUsedProf.
