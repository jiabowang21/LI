...
eachDayEnoughWorkers:- numWorkersNeededForDay(D,Num), findall( works(W, D), worker(W), Lits ), atLeast(Num, Lits), fail.
eachDayEnoughWorkers.

blocked:- isBlocked(W,D), writeClause([ -works(W,D) ]), fail.
blocked.

atMostThreeConsecutiveDays:- fourConsecutiveDays(D1,D2,D3,D4), worker(W),
                          writeClause( [ -works(W,D1), -works(W,D2), -works(W,D3), -works(W,D4) ] ), fail.
atMostThreeConsecutiveDays.
