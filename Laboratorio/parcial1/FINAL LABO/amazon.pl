...
eachDayEnoughWorkers:- numWorkersNeededForDay(D,Need), findall( works(W,D), worker(W), Lits ), atLeast(Need,Lits), fail.
eachDayEnoughWorkers.

blocked:- isBlocked(W,D), writeClause([ -works(W,D) ]), fail.
blocked.

atMostThreeConsecutiveDays:- worker(W), fourConsecutiveDays(D1,D2,D3,D4), writeClause([ -works(W,D1), -works(W,D2), -works(W,D3), -works(W,D4) ]), fail.
atMostThreeConsecutiveDays.

% For each each day D we need one clause, expressing that some of the supervisors works or noSupervisor(D) is true:
defineNoSupervisor:- day(D),  findall(works(W,D), supervisor(W), Lits),  writeClause( [noSupervisor(D)|Lits] ),  fail.
defineNoSupervisor.
...
%
costOfThisSolution(M,Cost):- findall(D,member(noSupervisor(D),M),L), length(L,Cost), !.
